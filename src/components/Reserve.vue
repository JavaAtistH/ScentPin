            ...
            <div class="steps-wrapper">
              <div v-for="(step, index) in steps" :key="index" class="step-item">
                <div class="step-circle" :class="[      
                  currentStep > index // 原来currentStep > index+1
                    ? 'step-completed'
                    : currentStep === index + 1
                      ? 'step-current'
                      : 'step-upcoming'
      ...
      <div class="form-content">
        ...
        <div v-if="currentStep === 1" class="step-container">
          ...
          <div class="age-slider">
            <div class="slider-track" ref="sliderContainerRef">
              ...
              <div class="slider-handle" :style="{ left: `${(age / 100) * 100}%` }" @mousedown="startDragging"
        ...
        <div v-if="currentStep === 2" class="step-container">
          ...
          <div class="gender-options">
            <button v-for="option in genderOptions" :key="option.value" @click="gender = option.value"
              class="gender-button" :class="{ 'selected': gender === option.value }">
              <div class="gender-content">
                <component :is="option.icon" class="gender-icon" :class="{ 'selected': gender === option.value }" />
                <span class="gender-label" :class="{ 'selected': gender === option.value }">
                  {{ option.label }}
                </span>
              </div>
            </button>
        ...
        <div v-if="currentStep === 3" class="step-container">
          ...
          <div class="strength-options">
            <button v-for="(option, index) in strengthOptions" :key="index" @click="strength = option.value"
              ...
              <div class="strength-bar" :class="{ 'selected': strength === option.value }"
                :style="{ height: `${option.height}%` }"></div>
              <div class="strength-label" :class="{ 'selected': strength === option.value }">
                {{ option.label }}
              </div>
            </button>
          </div>
...
.form-container {
  ...
  /* 增加顶部内边距，为header和progress bar留出空间 */
}
